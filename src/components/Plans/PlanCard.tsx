import { FiCheck, FiStar, FiShield, FiZap, FiLock, FiTrendingUp, FiAward, } from "react-icons/fi";
...
type Color = "indigo" | "purple" | "gold";

const COLOR_BY_PLAN: Record<number, Color> = {
...
const PlanCard: React.FC<PlanCardProps> = ({ plan, features = [], popular = false, onPurchase }) => {
  ...
  const getColorClasses = (color: Color, popular: boolean) => {
    if (popular) {
      return {
        border:
  ...
  const planColor: Color = COLOR_BY_PLAN[plan?.planType ?? PLAN_TYPES.BASIC] ?? "indigo";
  ...
  const oneTime = toEthStr(plan.oneTimePrice);
  const monthly = toEthStr(plan.monthlyPrice);
  const coverage = toEthStr(plan.coverageAmount);
  const deductible = toEthStr(plan.deductible);
    ...
    features.length > 0
      ...
      : [
          ...
          planName === "Premium" || planName === "Platinum" ? "Mental Health Coverage" : "Basic Mental Health",
