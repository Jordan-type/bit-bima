import { FiShield, FiFileText, FiDollarSign, FiTrendingUp, FiArrowUpRight, FiZap, FiLock, FiActivity, } from "react-icons/fi";
import { contractService, } from "../../services/contract";
import { CLAIM_STATUS, PLAN_TYPES, PAYMENT_TYPES, POLICY_STATUS,} from "../../constant/index";
import type { Policy, Claim, ContractStats } from "@/types/app";
...
// tiny helper that won’t explode on bigint
const toNum = (v: string | number | bigint | undefined) =>
...
const StatsCards: React.FC<StatsCardsProps> = ({
  ...
  const totalCoverage = userPolicies.reduce((sum, policy) => {
    if (policy.status === POLICY_STATUS.ACTIVE) {
      return sum + toNum(policy.coverageAmount);
    }
    return sum;
  }, 0);

  const totalPaid = userPolicies.reduce(
    (sum, policy) => sum + toNum(policy.totalPaid || 0),
    0
  );
  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      {stats.map((stat, index) => {
        const ChangeIcon = getChangeIcon(stat.changeType as "positive" | "negative" | "neutral");
