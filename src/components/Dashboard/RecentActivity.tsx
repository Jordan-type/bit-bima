...
import type { Policy, Claim, ClaimStatus } from "@/types/app";
import { toEthStr, tsToDateStr, celoscanBase, celoscanAddress, fmt, toTokenStr } from "@/utils/web3";
import { ComponentType } from "react";

type ActivityType = "policy_created" | "premium_paid" | "claim_submitted" | "claim_processed";
type ActivityColor = "indigo" | "green" | "yellow" | "red" | "purple";
...
const toSec = (v: string | number | bigint | undefined) =>
...
const RecentActivity: React.FC<RecentActivityProps> = ({
  ...
  const getActivities = () => {
    ...
    policies.forEach((policy) => {
      activities.push({
        ...
        timestamp: toSec(policy.startDate) || Math.floor(Date.now() / 1000),
        color: "indigo",
        amount: `${Number(policy.premium ?? 0).toFixed(4)} ETH`,
      });
      ...
      if (
        ...
        activities.push({
          ...
          timestamp: toSec(policy.lastPaymentDate),
          color: "green",
          amount: `${Number(policy.premium ?? 0).toFixed(4)} ETH`,
        });
      }
    ...
    claims.forEach((claim) => {
      activities.push({
        ...
        timestamp: toSec(claim.submissionDate || Date.now() / 1000),
        color: "yellow",
        amount: `${Number(claim.claimAmount ?? 0).toFixed(4)} ETH`,
        status: claim.status,
      });
      ...
      if (claim.processedDate && claim.processedDate !== "0") {
        activities.push({
          ...
          timestamp: toSec(claim.processedDate),
          color:
            claim.status === CLAIM_STATUS.APPROVED ||
            claim.status === CLAIM_STATUS.PAID
              ? "green"
          ...
          amount:
            claim.status === CLAIM_STATUS.APPROVED ||
            claim.status === CLAIM_STATUS.PAID
              ? `${Number(claim.approvedAmount ?? 0).toFixed(4)} ETH`
              : null,
        });
      }
    });
  ...
  const getColorClasses = (color: ActivityColor) => {
    const colors = {
      indigo:
