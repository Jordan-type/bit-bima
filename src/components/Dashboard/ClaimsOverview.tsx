...
import type { Claim, ClaimStatus } from "@/types/app";
...
const ClaimsOverview: React.FC<ClaimsOverviewProps> = ({
  claims = [],
  loading = false,
  onRefresh,
}) => {
  ...
  const getStatusIconColor = (status: ClaimStatus) => {
    const colors = {
  ...
  const formatDate = (timestamp: string | number | bigint) => {
    ...
    return new Date((typeof timestamp === "bigint" ? Number(timestamp) : Number(timestamp)) * 1000).toLocaleDateString();
  };
                  ...
                  <div className="relative grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div className="flex items-center space-x-2 p-3 bg-gradient-to-r from-blue-50/50 to-cyan-50/50 rounded-lg border border-blue-100/50">
                      ...
                      <span className="text-gray-700">
                        ...
                        {toNum(claim.claimAmount || 0).toFixed(4)} ETH
                      </span>
                    </div>
                    <div className="flex items-center space-x-2 p-3 bg-gradient-to-r from-purple-50/50 to-pink-50/50 rounded-lg border border-purple-100/50">
                      ...
                      <span className="text-gray-700">
                        ...
                        {formatDate(claim.submissionDate ?? 0)}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="mt-4 p-3 bg-gradient-to-r from-emerald-50 to-green-50 rounded-lg border border-emerald-200/50">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-semibold text-emerald-800">
                          Approved Amount:{" "}
                          {toNum(claim.approvedAmount || 0).toFixed(4)} ETH
                        </span>
                      </div>
                    </div>
                  )}
