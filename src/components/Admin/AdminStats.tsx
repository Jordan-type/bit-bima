...
type AdminStatsData = {
  contractBalance: bigint;     // ERC20 in token decimals
  totalPolicies: bigint;
  totalClaims: bigint;
  plans: Plan[];
  tokenSymbol: string;         // from getTokenMeta
  tokenDecimals: number;       // from getTokenMeta
};
...
const AdminStats: React.FC<AdminStatsProps> = ({ data, loading, onRefresh }) => {
  ...
  const dec = data?.tokenDecimals ?? 18;

  const stats = [
    {
      name: "Contract Balance",
      value: loading ? "..." : `${fmt(data.contractBalance, dec, 2)} ${symbol}`,
      description: `Total ${symbol} in RiskPool`,
      icon: FiDollarSign,
      color: "green",
      change: "+12.5%",
    ...
    {
      name: "Total Policies",
      value: loading ? "..." : Number(data.totalPolicies ?? 0n).toString(),
    ...
    {
      name: "Total Claims",
      value: loading ? "..." : Number(data.totalClaims ?? 0n).toString(),
    ...
    {
      name: "Active Plans",
      value: loading ? "..." : (data.plans?.filter((p) => p.isActive).length ?? 0).toString(),
  ...
  type ColorKey = "green" | "blue" | "purple" | "indigo" | "yellow" | "red";
  const getColorClasses = (color: ColorKey) => ({
    green:  { bg: "from-emerald-100 to-green-100", text: "text-emerald-600", border: "border-emerald-200", glow: "from-emerald-500/10 to-green-500/10" },
    blue:   { bg: "from-blue-100 to-cyan-100",     text: "text-blue-600",    border: "border-blue-200",    glow: "from-blue-500/10 to-cyan-500/10" },
    purple: { bg: "from-purple-100 to-pink-100",   text: "text-purple-600",  border: "border-purple-200",  glow: "from-purple-500/10 to-pink-500/10" },
    indigo: { bg: "from-indigo-100 to-purple-100", text: "text-indigo-600",  border: "border-indigo-200",  glow: "from-indigo-500/10 to-purple-500/10" },
    yellow: { bg: "from-yellow-100 to-orange-100", text: "text-yellow-600",  border: "border-yellow-200",  glow: "from-yellow-500/10 to-orange-500/10" },
    red:    { bg: "from-red-100 to-pink-100",      text: "text-red-600",     border: "border-red-200",     glow: "from-red-500/10 to-pink-500/10" },
  }[color]);

  const getChangeIcon = (t: "positive" | "negative" | "neutral") =>
    t === "positive" ? FiArrowUp : t === "negative" ? FiArrowDown : FiMinus;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => {
          const colorScheme = getColorClasses(stat.color as ColorKey);
